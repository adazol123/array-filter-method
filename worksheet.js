...
//***REDUCE***
    ...
    const characterByEyeColor = characters.reduce((acc, cur) => {
        ...
        if (acc[color]) {
            acc[color] = acc[color] +1; // acc[color] ++;
        }else {
            acc[color] = 1;
        } return acc;
    }, {})
...
//***FILTER***
    ...
    const greated100Characters = characters.filter((character) => {
        return character.mass > 100;
    });
    console.log(greated100Characters);
...
//***SOME*** return boolean whether the condition is meet or not
